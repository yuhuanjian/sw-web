  ...
  <div ref="main" class="dashboard-container">
    ...
    <div ref="action" class="action-wrap">
      <div>
        ...
        >新增异常</el-button> -->
        <el-button type="primary" icon="el-icon-upload2" @click="exportASFClick" >非洲猪瘟检测登记导出</el-button>
        <el-button type="primary" icon="el-icon-upload2" @click="exportClenbuterolClick" >瘦肉精检测登记导出</el-button>
      </div>
      <div>
        <el-button
          v-has="'quarantineMana:export'"
          type="primary"
    ...
    <TableWrap>
      ...
      >
        ...
        <el-table-column label="检疫状态" min-width="150">
          <template slot-scope="scope">
            <p v-if="scope.row.quarantineStatus === 1" class="root-status-f">
              <span /> {{ scope.row.state }}
            </p>
            <p v-if="scope.row.quarantineStatus === 2" class="root-status-s">
              <span /> {{ scope.row.state }}
            </p>
            <p v-if="scope.row.quarantineStatus === 3" class="root-status-r">
              <span /> {{ scope.row.state }}
            </p>
        ...
        <el-table-column fixed="right" label="操作" width="150">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              "
              v-has="'quarantineMana:abnormalInput'"
              type="text"
              size="small"
      ...
      <div class="quarantine-xq22">
        ...
        >
          ...
          >
            ...
            >
              <el-option label="准入检疫" value="1" />
              <el-option
                v-if="quarantineStatus >= 1"
                label="准宰检疫"
                value="2"
              />
              <el-option
                v-if="quarantineStatus >= 2"
                label="准出检疫"
                value="3"
              />
            </el-select>
          ...
          >
            ...
            >
              ...
              />
              <el-option
                v-if="ruleForm.sequence == 3"
                label="检疫不合格"
                value="5"
              />
              <el-option v-if="ruleForm.sequence == 3" label="病料" value="6" />
            </el-select>
          </el-form-item>
          <el-form-item
            v-if="
              ruleForm.sequence == 2 ||
                (ruleForm.sequence == 3 && ruleForm.abnormalType != 6)
            "
            label="生猪编号:"
            :label-width="formLabelWidth"
            prop="livePigNum"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        sequence: [
          { required: true, message: '请选择检疫阶段', trigger: 'change' }
        ],
        abnormalType: [
  ...
  methods: {
    ...
    //     `enterTimeStart=${this.formSearch.enterTime}&farmName=${this.formSearch.farmName}&orderNum=${this.formSearch.orderNum}&pickupOddNum=${this.formSearch.pickupOddNum}&pickupPlateNumbers=${this.formSearch.pickupPlateNumbers}&quarantineStatus=${this.formSearch.quarantineStatus}&shipper=${this.formSearch.shipper}&wholesaler=${this.formSearch.wholesaler}&quarType=null`
    //   )
    // },
    exportZY: _.debounce(function() {
      window.location.href = GetQuarantineexportQuarantineInfo(
        ...
        }&wholesaler=${this.formSearch.wholesaler}&quarType=null&branchId=${this.formSearch.branchId}`
      )
    }, 500),
    exportClenbuterolClick: _.debounce(function() {
      if(this.SelectionList.length > 0) {
        window.location.href = exportClenbuterol(`ids=`+this.ids);
      } else {        
    ...
    exportASFClick: _.debounce(function() {
      if(this.SelectionList.length > 0) {
        window.location.href = exportASF(`ids=`+this.ids);
      } else {        
        this.$message.error('请至少选择一项再进行操作');
      }
    },500),
    addAbnormal(obj, formName) {
      obj.quarantineStatus <= 3
        ...
        : obj.quarantineStatus > 3 && obj.quarantineStatus <= 6
          ? (this.quarantineStatus = 1)
          : obj.quarantineStatus > 6 && obj.quarantineStatus <= 9
            ? (this.quarantineStatus = 2)
            : ''
      this.ruleForm = {
        abnormalType: '',
        livePigNum: '',
      ...
      this.practicalEnterNum = obj.practicalEnterNum
      // this.dialogFormVisible = true
    },
    reset() {
    ...
    handleClick(row) {
      ...
      //   'detailsMessage',
      //   `id=${row.id}&orderNum=${row.orderNum}&farmName=${row.farmName}&shipper=${row.shipper}&wholesaler=${row.wholesaler}&enterTime=${row.enterTime}&practicalEnterNum=${row.practicalEnterNum}&abnormalNum=${row.abnormalNum}&allowKillNum=${row.allowKillNum}&qualifiedNum=${row.qualifiedNum}&quarantineStatus=${row.quarantineStatus}&state=${row.state}&pickupOddNum=${row.pickupOddNum}&pickupPlateNumbers=${row.pickupPlateNumbers}`
      // )
      var detailsMessage = {
        id: row.id,
      ...
      localStorage.setItem('detailsMessage', JSON.stringify(detailsMessage))
      this.$router.push({
        path: 'details',
        query: {
          id: row.id,
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          postQuarantineSaveAbnormalInfo({
            ...
            abnormalNum: Number(this.ruleForm.abnormalNum),
            disposeManner: this.ruleForm.disposeManner,
            epidemicMaterials: this.ruleForm.epidemicMaterials,
            livePigNum:
              livePigNumList.length === 0 ? '' : livePigNumList.join(' '),
            orderId: this.orderId,
            sequence: Number(this.ruleForm.sequence),
